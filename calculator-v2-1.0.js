...
function requestCrossDomain( site, callback ) {
	...
	var yql = 'http://query.yahooapis.com/v1/public/yql?q=' + encodeURIComponent('select * from htmlstring where url="' + site + '"') + '&format=xml&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys&callback=?';
	...
	function cbFunc(data) {
		...
		if ( data.results[0] ) {
			...
			//my own stripping of <body> etc
			//data = data.replace(/<([a-z])+>|<([a-z]+\/)>|<(\/[a-z]+)>/gm,'');
...
const leveldifference = [0,200,300,400,500,0]; //0,1,2,3,4,5. if level 1, needs 200 to level up, etc. 5 needs 0
        ...
        $(document).ready(function(){
			  ...
			  request.attr({id:"xprequest",name:"xprequest",type:"button",disabled:"disabled",style:"float:right;",onclick:"submitform(this);"});
              ...
              //formp.append("Points & item calculator (v2.22 beta - Insignia addition)");
              ...
              var radioyes = $(document.createElement('input')).attr({type:"radio",id:"ironmany",name:"ironman",value:"yes"});
              var radiono = $(document.createElement('input')).attr({type:"radio",id:"ironmann",name:"ironman",value:"no",checked:"checked"});
              ...
              calculate.append("Calculate");
		  request.attr({type:"button",name:"pointsrequest",id:"pointsrequest",style:"float:right;",disabled:"disabled",onclick:"submitform(this);"});
			  request.append("Request this leech");
              left.append($(document.createElement('br')));
			  left.append(request);
			  left.append(calculate);
	...
	function calculateXP(level,bxp,unlock,skill){
		...
		if(unlock==1){
			var tempxp = bxp-FHM;
			$("#breakdownxp").append("1x&nbsp; 1-9HM &nbsp;" + commaSeparateNumber(FULLHM));
			$("#breakdownxp").append($(document.createElement('br')));
			$("#counter").val(parseInt($("#counter").val())+parseInt(FHM));
			return FULLHM+calculateXP(level,tempxp,2,skill);;
		}
		if(unlock==2){
	...
	function fetchlevel(name,skill){
		...
		//get stats from site
		requestCrossDomain('http://services.runescape.com/m=hiscore/index_lite.ws?player='+name, function(results) {
		$('#hiddenrs').html(results);
		$('#hiddenrs').hide();
		$('#skill_levels').hide();
		var array = $("#hiddenrs").text().split(',');
		var level=1;
		//alert(array[10]);
		var agility =array[35];
		var mining = array[31];
		var firemaking = array[25];
		$("#hlevels").text("");//reset it
...
function calculateinsignia(points,role,kingskilled){
    ...
    if(newunlock==1){
        temp = temp-FHM;
        newunlock=2;
        $("#breakdown").append("1x&nbsp; 1-9HM &nbsp;" + commaSeparateNumber(FULLHM));
        $("#breakdown").append($(document.createElement('br')));
        $("#unlockstatus").val(2);
